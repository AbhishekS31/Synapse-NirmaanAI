...
export const PreviewPane: FC = () => {
  ...
  const generateIframePreview = async () => {
    ...
    // Include all component files before App.jsx
    const componentsCode = componentFiles.map(file => file.content).join('\n\n');
    ...
    // Use htmlFile content as the base if available, or create a default structure
    const baseHtml = htmlFile.content || `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        <meta charset="UTF-8">
    ...
    const fullHtml = baseHtml
      .replace('</head>', `${tailwindCDN}${reactCDN}${cssContent}</head>`)
  ...
  const generateHtmlPreview = () => {
    ...
    let htmlContent = (htmlFile?.content || '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body><div id="root"></div></body></html>')
      .replace('</head>', `${tailwindCDN}${cssContent}</head>`);
    
    // Find UI components from files
    const componentFiles = generatedFiles.filter(f => 
      (f.path.includes('component') || f.path.includes('Component') || f.path.includes('App')) && 
      (f.path.endsWith('.jsx') || f.path.endsWith('.tsx'))
    );
    ...
    if (appFile) {
      // Try to extract the main JSX content from App component
      const jsxMatches = appFile.content.match(/return\s*\(\s*([\s\S]*?)\s*\);?\s*\}/);
      if (jsxMatches && jsxMatches[1]) {
        let jsx = jsxMatches[1]
          .replace(/className=/g, 'class=')
          .replace(/\{\/\*[\s\S]*?\*\/\}/g, '') // Remove comments
          .replace(/\{[^{}]*\}/g, ''); // Remove simple JS expressions in {}
    ...
    if (!componentContent) {
      // Fallback to extracting from individual components
      for (const file of componentFiles) {
        const jsxMatches = file.content.match(/return\s*\(\s*([\s\S]*?)\s*\);?\s*\}/);
        if (jsxMatches && jsxMatches[1]) {
          const jsx = jsxMatches[1]
            .replace(/className=/g, 'class=')
            .replace(/\{\/\*[\s\S]*?\*\/\}/g, '') // Remove comments
    ...
    htmlContent = htmlContent.replace('<div id="root"></div>', `<div id="root">${componentContent || '<div class="p-4">No preview content could be extracted. Please check console for errors.</div>'}</div>`);
